...
%--------------------------------------------------------
%  Updated to enable setting the colormap  
%  Updated with fig pos setting 
...
for row_index = 1:num_rows,
    for col_index = 1:num_cols,
        ...
        if scale ==0
            imshow(image_matrix(:,:,curr_index),[]); colormap(gca,cmap);
        elseif size(scale,1)*size(scale,2)<3
            imshow(image_matrix(:,:,curr_index), scale); colormap(gca,cmap);
        else
            imshow(image_matrix(:,:,curr_index), scale(curr_index,:)); colormap(gca,cmap);
        end
        if ~isempty(titles),
